...
export const filterByTenantRead: Access = (args) => {
    ...
    if (selectedTenant) {
        ...
        // give them read access to only posts for that tenant
        if (superAdmin) {
            return {
                tenant: {
...
export const canMutatePost: Access = (args) => {
    const req = args.req
    const superAdmin = isSuperAdmin(args)
    ...
    // super admins can mutate posts for any tenant
    if (superAdmin) {
        return true
    }
